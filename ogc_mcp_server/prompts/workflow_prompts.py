...
🎯 工具执行流程(一定要执行工具，不能描述工具返回的信息，每个工具只调用一次)：
1. search_and_list_geographic_data() - 搜索相关数据，每次任务必须执行
2. add_wms_layer() - 添加底图
3. add_wfs_layer() - 添加WFS图层(如果用户查询需要过滤条件，一定要过滤)
4. create_composite_visualization() - 创建可视化，在图层添加完才能执行
